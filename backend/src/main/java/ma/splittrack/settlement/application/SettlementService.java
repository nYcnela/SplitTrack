...
public class SettlementService {
    ...
    @Transactional(readOnly = true)
    public List<Settlement> listForExport(Scope scope, YearMonth month) {
        if (scope == Scope.CYCLE) {
            throw new IllegalArgumentException("scope=CYCLE is not supported for settlements export");
        }
        ...
        if (scope == Scope.MONTH) {
            ...
            OffsetDateTime start = month.atDay(1).atStartOfDay().atOffset(ZoneOffset.UTC);
            OffsetDateTime end = month.plusMonths(1).atDay(1).atStartOfDay().atOffset(ZoneOffset.UTC);
            return settlementRepository.findByCreatedAtBetween(start, end);
        }
